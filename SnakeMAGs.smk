...
rule quality_filtering:
    input:
        ...
        rev=READS2,
    output:
        quality_passed_R1="{smp}/QC_fq/quality_filtering/{smp}-QUALITY_PASSED_R1.fastq",
        quality_passed_R2="{smp}/QC_fq/quality_filtering/{smp}-QUALITY_PASSED_R2.fastq",
    conda:
        config["conda_env"] + "IU.yaml"
    benchmark:
...
rule adapter_trimming:
    ...
    output:
        ...
        un_trimmed_1="{smp}/QC_fq/adapter_trimming/{smp}_1un.trimmed.fastq",
        trimmed_2="{smp}/QC_fq/adapter_trimming/{smp}_2.trimmed.fastq",
        un_trimmed_2="{smp}/QC_fq/adapter_trimming/{smp}_2un.trimmed.fastq",
    conda:
        config["conda_env"] + "TRIMMOMATIC.yaml"
    benchmark:
...
rule depth_file:
    ...
    conda:
        config["conda_env"] + "METABAT2.yaml"
    benchmark:
        "{smp}/benchmarks/depth_file.benchmark.txt"
    resources:
        mem=config["resources_binning"],
    log:
...
rule classification:
    ...
    conda:
        config["conda_env"] + "GTDBTK.yaml"
    benchmark:
        "{smp}/benchmarks/classification.benchmark.txt"
    params:
        GTDB=config["GTDB_data_ref"],
    threads: config["threads_gtdb"]
    resources:
        mem=config["resources_gtdb"],
    log:
